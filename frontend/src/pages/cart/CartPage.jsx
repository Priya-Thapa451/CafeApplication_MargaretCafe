...
const CartPage = () => {
  ...
  if (loading)
    return (
      <p className="text-center mt-10 text-[#4b2e2b]">Loading cart...</p>
    );
  if (error)
    return (
  ...
  return (
    <div className="min-h-screen bg-[#f9f4ef] flex flex-col items-center py-10 px-4 sm:px-6">
      {showCheckout ? (
        // Display OrderForm once checkout is triggered
        <div className="w-full max-w-3xl mt-8">
          <OrderForm cartItems={cartItems} totalAmount={totalPrice} />
        </div>
      ) : (
        // Show Cart page when checkout is not triggered
        <>
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg p-6">
              {cartItems.map((item) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-lg text-[#4b2e2b]">
                        {item.menu.name}
                      </h4>
                      <p className="text-[#8a6e63] text-sm">
                        RS {item.menu.price.toFixed(2)} each
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center border rounded-md overflow-hidden">
                      ...
                      <span className="px-4 py-1 text-[#4b2e2b]">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() =>
              ...
              <div className="text-right text-xl font-semibold text-[#4b2e2b] mt-6">
                <p>Total: RS {totalPrice.toFixed(2)}</p>
              </div>

              <button
                onClick={() => setShowCheckout(true)}
                className="mt-4 w-full bg-[#4b2e2b] hover:bg-[#3b221e] text-white font-medium py-2 px-4 rounded-lg transition duration-200"
              >
                Proceed to Checkout
              </button>
            </div>
